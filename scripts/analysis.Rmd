...
### load random sample
load(file = "~/tubework/projects/sample-cluster/r-sample-cluster/data/channel_data_2025-09-22.RData")
...
tmp <- video_data %>%
  ...
  mutate(
    tmp = str_match(duration, "PT(?:(\\d+)H)?(?:(\\d+)M)?(?:(\\d+)S)?") %>% 
           as.data.frame() %>%
  transmute(
    hours   = as.numeric(V2) %>% replace_na(0),
    minutes = as.numeric(V3) %>% replace_na(0),
    seconds = as.numeric(V4) %>% replace_na(0)
  ) %>%
    mutate(
  ...
  mutate("duration_minutes" = unlist(tmp)) %>%
  select(-tmp) %>%
  left_join(
  ...
  mutate(mean_dur_topic = mean(duration_minutes, na.rm = TRUE),
         sd_dur_topic= sd(duration_minutes,  na.rm=TRUE),
         z_duration = (duration_minutes - mean_dur_topic)/sd_dur_topic) %>%
  ungroup() %>%
  group_by(channel_id) %>%
  mutate(mean_z_duration = mean(z_duration, na.rm = TRUE)) %>%
  ungroup() %>%
  select(mean_z_duration, channel_id) %>%
  distinct()
...
rm(video_data,tmp, tmp2, tmp3)
```


```{r build our samples}
# including cases with unknown or mixed categories in socio-structural variables
df_all <- channel_data %>% 
  ...
  mutate(
    inc_monthly = ifelse(is_monetized == "yes", inc_monthly, 0)) %>%
  group_by(channel_topic) %>%
  mutate(n_topic = n()) %>%
  ungroup()

# excluding cases with unknown or mixed categories in socio-structural variables
df_cut <- df_all %>%
  filter(race != "unknown") %>%
  filter(age != "unknown") %>%
  filter(sex != "unknown") %>%
  ...
  mutate(
    inc_sources = ifelse(is_monetized == "yes", inc_sources - 1, inc_sources)
  ) %>%
  group_by(channel_topic) %>%
  mutate(n_topic = n()) %>%
  ungroup()
...
```{r cluster analysis: 3 clusters}
df_cluster <- df_cut %>%
  # filter(last_video < (365*1)) %>%
  transmute(
    ...
    mean_z_dur_std = scale(mean_z_duration)[,1],
...
Factoshiny(df_cluster)

res_pca<-PCA(df_cluster,ncp=Inf, scale.unit=FALSE,quali.sup=c(4,11),graph=FALSE)
res_hcpc<-HCPC(res_pca,nb.clust=3,consol=FALSE,graph=FALSE)

# res_hcpc$call

df_cut$cluster <- res_hcpc[["data.clust"]]$clust

table(df_cut$cluster)
table(df_cut$cluster, df_cut$is_monetized)

df_cut %>%
  group_by(cluster) %>%
  summarise(
    # mean_inc_sources = mean(inc_sources),
    median_inc_sources = median(inc_sources),
    mean_inc = mean(inc_monthly),
    mean_subs = mean(subscriber_count),
    mean_activity = mean(active_months),
    median_activity = median(active_months),
    mean_last_video = mean(last_video),
    median_last_video = median(last_video)
  ) %>%
  print()

df_cut %>%
  filter(cluster == 1) %>%
  filter(is_monetized == "yes") %>%
  summarise(mean_inc = mean(inc_monthly))
```


```{r cluster analysis: 4 cluster}
df_cluster <- df_cut %>%
  # filter(last_video < (365*1)) %>%
  transmute(
    subscriber_count_std = scale(subscriber_count)[,1],
...
cluster <- Factoshiny(df_cluster)

res.PCA<-PCA(df_cluster,ncp=Inf, scale.unit=FALSE,quali.sup=c(4,12),graph=FALSE)
res.HCPC<-HCPC(res.PCA,nb.clust=4,consol=FALSE,graph=FALSE)

df_cut$cluster <- res.HCPC[["data.clust"]]$clust
...
```{r cluster analysis with log transformations}
df_cluster <- df_cut %>%
  # filter(last_video < (365*1)) %>%
  transmute(
    subscriber_count_std = scale(log(subscriber_count))[,1],
    # subscriber_count_std = scale(subscriber_count)[,1],
    inc_sources_std = scale(log(inc_sources+1))[,1],
    inc_monthly_std = scale(log(inc_monthly + 1))[,1],
    mail,
    active_months_std = scale(active_months)[,1],
    # channel_age_std = scale(channel_age)[,1], #
    mean_z_dur_std = scale(mean_z_duration)[,1],
    productivity_std = scale(log(productivity))[,1],
    # video_count_std = scale(video_count)[,1], #
    last_video_std = scale(log(last_video+1))[,1],
    audience_std = scale(audience)[,1],
    # sentiment_std = scale(mean_sentiment)[,1],
    hate_std = scale(log(hate_only_mean + 1))[,1],
    n_topic_std = scale(n_topic)[,1], #
    channel_topic,
  ) %>%
  na.omit()
...
tmp <- data.frame(
  cluster = 1:20,
  within_inertia = res_hcpc$call$t$within[1:20]
)
...
ggplot(
  ...
  labs(x = "number of clusters", y = "within inertia") +
  theme(
    panel.grid.minor.x = element_blank()
  )

ggsave(filename = "graphics/elbow_plot.png",
       plot = last_plot(),
       width = 12,
       height = 8,
       dpi = 300)
...
tmp <- data.frame(
  cluster = 1:10,
  inertia = res.HCPC$call$t$inert.gain[1:10]
)
...
ggsave(filename = "graphics/cluster_elbow_plot2.png",
       plot = last_plot(),
       width = 12,
       height = 8,
       dpi = 300)
...
df_cut %>%
  ...
  mutate(
    cluster = case_match(cluster,
                       "1" ~ paste("Hope Labor (N =",sum(df_cut$cluster == 1), ")",sep = ""),
                       "2" ~ paste("Adapted Survivors (N =",sum(df_cut$cluster == 2), ")",sep = ""),
                       "3" ~ paste("Top Earners (N =",sum(df_cut$cluster == 3), ")",sep = "")
    )
  ) %>%
  mutate(cluster = factor(cluster,
                        levels = c(
                          paste("Hope Labor (N =",sum(df_cut$cluster == 1), ")",sep = ""),
                          paste("Adapted Survivors (N =",sum(df_cut$cluster == 2), ")",sep = ""),
                          paste("Top Earners (N =",sum(df_cut$cluster == 3), ")",sep = "")
                        ),
                        ordered = TRUE)
         ) %>%
  ggplot() +
  ...
  labs(
    ...
    size = "workload per video",
    # size = "exposure to hate speech",
    caption = paste("Note: data based on N =", format(nrow(df_cut), big.mark = ","), "channels")
  ) + 
  facet_wrap(~ cluster,
             # scales = "free"
             ) +
...
```{r cluster analysis hope labour}
df_cut_hl <- df_cut %>%
  filter(cluster %in% c(1)) 

df_cluster_hl <- df_cut_hl %>%
  # filter(last_video < (365*1)) %>%
  transmute(
    subscriber_count_std = scale(subscriber_count)[,1],
    inc_sources_std = scale(inc_sources)[,1],
...
Factoshiny(df_cluster_hl)

res_pca<-PCA(df_cluster_hl,ncp=Inf, scale.unit=FALSE,quali.sup=c(4,11),graph=FALSE)
res_hcpc<-HCPC(res_pca,nb.clust=3,consol=FALSE,graph=FALSE)

# res_hcpc$call

df_cut_hl$cluster <- res_hcpc[["data.clust"]]$clust

table(df_cut_hl$cluster)
table(df_cut_hl$cluster, df_cut_hl$is_monetized)
...
tmp <- df_cut %>%
  mutate(
    cluster_name = case_match(cluster,
                       "1" ~ "Hope Labour",
                       "2" ~ "Adapted Survivors",
                       "3" ~ "Top Earners"
    )
...
summary(model_soc)
tidy_sum <- tidy(model_soc, conf.int = TRUE)

write.xlsx(tidy_sum, file = "regresssion_results.xlsx")

tbl_regression(model_soc, exp = TRUE)

exp(coef(model_soc))

# The multinomial logit for females relative to males is 0.02 units lower for being part of the second cluster compared to being part of the first cluster, given all other predictor variables in the model are held constant. In other words, females are less likely than males to be in cluster 2 than in cluster 1.

stargazer(
  model_soc,
  type = "html",
  # coef = list(exp(coef(model_soc))),
  coef = list(coef(model_soc)),
  p.auto = FALSE,
  # out = "model_soc_or.html"
  out = "model_soc.html"
  )
...
```{r size of categories}

table(tmp$cluster_name, tmp$age)
table(tmp$cluster_name, tmp$sex)
table(tmp$cluster_name, tmp$race)
table(tmp$cluster_name, tmp$channel_topic)
...
ggplot(ame_social) +
  ...
  guides(
    color = guide_legend(direction = "vertical", reverse = TRUE),
  )

ggsave(
  filename = "~/tubework/projects/sample-cluster/r-sample-cluster/graphics/ame-plot.png",
  plot = last_plot(),
  width = 10,
  height = 6,
  dpi = 600
...
# Predicted probabilities für alle Personen
preds <- predict(model_soc, type = "probs")
preds_df <- as.data.frame(preds)

# Baseline probability (Durchschnitt über alle Personen)
colMeans(preds)

# join the prediction to the original dataframe
df_preds <- cbind(tmp, preds)


### Gender

# ggeffect(model_soc, terms = "sex") %>%
#   plot()


# df_long <- df_preds %>%
#   group_by(sex) %>%
#   summarise(across(colnames(preds), mean)) %>%
#   tidyr::pivot_longer(-sex, names_to = "cluster", values_to = "prob")
# 
# ggplot(df_long, aes(x = cluster, y = prob, fill = factor(sex))) +
#   geom_col(position = "dodge") +
#   labs(x = "Cluster", y = "Predicted Probability", fill = "Sex") +
#   theme_minimal()

# manual calculation of predicted probabilities
pp <- predictions(model_soc,
                  newdata = datagrid(sex = c("male","female")),
                  type = "probs")

ggplot(pp, aes(x = group, y = estimate, col = sex)) +
  geom_point(position = position_dodge(width = 0.9), size = 3) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high),
                size = 1,
                position = position_dodge(width = 0.9), width = 0.2) +
  labs(y = "Predicted probability", x = "Cluster") +
...
ggsave(
  filename = "~/tubework/projects/sample-cluster/r-sample-cluster/graphics/pp_sex.png",
  plot = last_plot(),
  width = 6,
  height = 6,
  dpi = 600
...
# ggeffect(model_soc, terms = "race") %>%
#   plot()

# df_long <- df_preds %>%
#   group_by(race) %>%
#   summarise(across(colnames(preds), mean)) %>%
#   tidyr::pivot_longer(-race, names_to = "cluster", values_to = "prob")
# 
# ggplot(df_long, aes(x = cluster, y = prob, fill = factor(race))) +
#   geom_col(position = "dodge") +
#   labs(x = "Cluster", y = "Predicted Probability", fill = "Race") +
#   theme_minimal()

pp <- predictions(model_soc,
                  newdata = datagrid(race = c("BIPoC","white")),
                  type = "probs")

ggplot(pp, aes(x = group, y = estimate, col = race)) +
  geom_point(position = position_dodge(width = 0.9), size = 3) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high),
                size = 1,
                position = position_dodge(width = 0.9), width = 0.2) +
  ...
  theme(axis.text.x = element_text(angle = 35, hjust = 1),
        ...
        legend.title = element_text(hjust = 0.5))
ggsave(
  filename = "~/tubework/projects/sample-cluster/r-sample-cluster/graphics/pp_race.png",
  plot = last_plot(),
  width = 6,
  height = 6,
  dpi = 600
...
# ggeffect(model_soc, terms = "age") %>%
#   plot()

# df_long <- df_preds %>%
#   group_by(age) %>%
#   summarise(across(colnames(preds), mean)) %>%
#   tidyr::pivot_longer(-age, names_to = "cluster", values_to = "prob")
# 
# ggplot(df_long, aes(x = cluster, y = prob, fill = factor(age))) +
#   geom_col(position = "dodge") +
#   labs(x = "Cluster", y = "Predicted Probability", fill = "Age") +
#   theme_minimal()


pp <- predictions(model_soc,
                  newdata = datagrid(age = c("under 20 yrs.","21 - 30 yrs.","31 - 40 yrs.","40+ yrs.")),
                  type = "probs")

ggplot(pp, aes(x = group, y = estimate, col = age)) +
  geom_point(position = position_dodge(width = 0.9), size = 3) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high),
                size = 1,
                position = position_dodge(width = 0.9), width = 0.2) +
  ...
  theme(#axis.text.x = element_text(angle = 35, hjust = 1),
        ...
        legend.title = element_text(hjust = 0.5))
ggsave(
  filename = "~/tubework/projects/sample-cluster/r-sample-cluster/graphics/pp_age.png",
  plot = last_plot(),
  width = 12,
  height = 6,
  dpi = 300
...
##### combine clusters in logistic regression 

```{r}

tmp <- df_cut %>%
  mutate(
    cluster_bin = ifelse(cluster == 1,1,0)
  )

tmp <- df_cut %>%
  mutate(
    cluster_bin = ifelse(cluster == 2,1,0)
  )

tmp <- df_cut %>%
  mutate(
    cluster_bin = ifelse(cluster %in% c(1,2),1,0)
  )

tmp <- df_cut %>%
  mutate(
    cluster_bin = ifelse(cluster == 3,1,0)
  )

tmp <- df_cut %>%
  mutate(
    cluster_bin = ifelse(cluster == 4,1,0)
  )

tmp <- df_cut %>%
  mutate(
    cluster_bin = ifelse(cluster == 5,1,0)
  )

tmp <- df_cut %>%
  mutate(
    cluster_bin = ifelse(cluster %in% c(4,5),1,0)
  )

tmp <- df_cut %>%
  mutate(
    cluster_bin = ifelse(cluster %in% c(4,5),1,
                         ifelse(cluster %in% c(1,2),0,NA))
  ) %>% 
  na.omit()

tmp <- df_cut %>%
  mutate(
    cluster_bin = ifelse(cluster %in% c(3),1,
                         ifelse(cluster %in% c(2),0,NA))
  ) %>% 
  na.omit()
