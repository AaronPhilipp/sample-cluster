...
```{r cluster analysis: 3 clusters without channel age}
df_cluster <- df_cut %>%
...
res.PCA<-PCA(df_cluster,ncp=Inf, scale.unit=FALSE,quali.sup=c(4,11),graph=FALSE)
...
```{r cluster analysis: 5 clusters intensity try}
df_cluster <- df_cut %>%
  mutate(intensity = active_months/channel_age) %>%
  # filter(last_video < (365*.5)) %>%
  transmute(
    subscriber_count_std = scale(subscriber_count)[,1],
    inc_sources_std = scale(inc_sources)[,1],
    inc_monthly_std = scale(inc_monthly)[,1],
    mail,
    intensity_count_std = scale(intensity)[,1],
    active_months_std = scale(active_months)[,1],
    # channel_age_std = scale(channel_age)[,1], #
    productivity_std = scale(productivity)[,1],
    # video_count_std = scale(video_count)[,1], #
...
df_cut %>%
  ...
  #                      "1" ~ paste("Hope Labour (N =",sum(df_cut$cluster == 1), ")",sep = ""),
  #                      "2" ~ paste("High-Risk Communicators (N =",sum(df_cut$cluster == 2), ")",sep = ""),
  #                      "3" ~ paste("Adapted Survivors (N =",sum(df_cut$cluster == 3), ")",sep = ""),
  #                      "4" ~ paste("Top Earners (N =",sum(df_cut$cluster == 4), ")",sep = "")
  #   )
  # ) %>%
  # mutate(cluster = factor(cluster,
  #                       levels = c(
  #                         paste("Hope Labour (N =",sum(df_cut$cluster == 1), ")",sep = ""),
  #                         paste("High-Risk Communicators (N =",sum(df_cut$cluster == 2), ")",sep = ""),
  #                         paste("Adapted Survivors (N =",sum(df_cut$cluster == 3), ")",sep = ""),
  #                         paste("Top Earners (N =",sum(df_cut$cluster == 4), ")",sep = "")
  #                       ),
  #                       ordered = TRUE)
  #        ) %>%
  mutate(
    cluster = case_match(cluster,
                       "1" ~ paste("Hope Labour Cemetry (N =",sum(df_cut$cluster == 1), ")",sep = ""),
                       "2" ~ paste("Hope Labour (N =",sum(df_cut$cluster == 2), ")",sep = ""),
                       "3" ~ paste("High-Risk Communicators (N =",sum(df_cut$cluster == 3), ")",sep = ""),
                       "4" ~ paste("Adapted Survivors (N =",sum(df_cut$cluster == 4), ")",sep = ""),
                       "5" ~ paste("Top Earners (N =",sum(df_cut$cluster == 5), ")",sep = "")
    )
  ) %>%
  mutate(cluster = factor(cluster,
                        levels = c(
                          paste("Hope Labour Cemetry (N =",sum(df_cut$cluster == 1), ")",sep = ""),
                          paste("Hope Labour (N =",sum(df_cut$cluster == 2), ")",sep = ""),
                          paste("High-Risk Communicators (N =",sum(df_cut$cluster == 3), ")",sep = ""),
                          paste("Adapted Survivors (N =",sum(df_cut$cluster == 4), ")",sep = ""),
                          paste("Top Earners (N =",sum(df_cut$cluster == 5), ")",sep = "")
                        ),
                        ordered = TRUE)
         ) %>%
  ggplot() +
...
tmp <- df_cut %>%
  mutate(
    cluster_name = case_match(cluster,
                       "1" ~ "Hope Labour Cemetry",
                       "2" ~ "Hope Labour",
                       "3" ~ "High-Risk Communicators",
                       "4" ~ "Adapted Survivors",
                       "5" ~ "Top Earners"
    )
  ) %>%
  ...
  # mutate(
  #   cluster_new = ifelse(cluster %in% c(1,2),1,cluster)
  # )
  # mutate(
  #   cluster_new = ifelse(cluster %in% c(4,5) , 4,cluster)
  # ) %>% filter(cluster_new != 5) %>% filter(cluster_new != 1)
  # filter(cluster_name != "Hope Labour Cemetry")

table(tmp$cluster_new)
...
# pred_class <- apply(preds, 1, function(x) colnames(preds)[which.max(x)])
# pred_class_df <- as.data.frame(pred_class)
...
# pred_by_female <- margins(model_soc, variables = "sex", type = "probs", at = list(sex = c("female","male")))
# summary(pred_by_female)
# plot(pred_by_female)
...
# Direkter Vergleich (Kontraste) zwischen male und female
cmp <- comparisons(model_soc, variables = "sex", type = "probs")
summary(cmp)
