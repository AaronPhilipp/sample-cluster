...
tmp <- video_data %>%
  ...
  ungroup() %>%
  select(-channel_topic) %>%
  rename("last_video" = "max") %>%
  mutate(last_video = abs(last_video - today()))
...
cramersV(channel_data$age, channel_data$channel_age)
cramersV(channel_data$sex, channel_data$age)
cramersV(channel_data$sex, channel_data$race)

# chisq.test(table(channel_data$race, channel_data$sex))
...
tmp <- df_cluster %>%
  transmute(
    ...
    # prod_comp_topic_std = scale(prod_comp_topic)[,1],
    sex,
    age,
    race,
    channel_topic,
...
# res.PCA<-PCA(tmp,ncp=Inf, scale.unit=FALSE,quali.sup=c(8,9,10,11,12,13),graph=FALSE)
# res.HCPC<-HCPC(res.PCA,nb.clust=3,consol=FALSE,graph=FALSE)

# res.PCA<-PCA(tmp,ncp=Inf, scale.unit=FALSE,quali.sup=c(9,10,11,12,13,14),graph=FALSE)
# res.HCPC<-HCPC(res.PCA,nb.clust=5,consol=FALSE,graph=FALSE)

# res.PCA<-PCA(tmp,ncp=Inf, scale.unit=FALSE,quali.sup=c(11,12,13,14,15,16,17),graph=FALSE)
# res.HCPC<-HCPC(res.PCA,nb.clust=3,consol=FALSE,graph=FALSE)


# with channel_age
res.PCA<-PCA(tmp,ncp=Inf,scale.unit=FALSE,quali.sup=c(11,12,13,14,15,16),graph=FALSE)
res.HCPC<-HCPC(res.PCA,nb.clust=3,consol=FALSE,graph=FALSE)
...
```{r cluster analysis only channel metrics}
...
res.PCA<-PCA(tmp,ncp=Inf, scale.unit=FALSE,quali.sup=c(11,12,13),graph=FALSE)
res.HCPC<-HCPC(res.PCA,nb.clust=3,consol=FALSE,graph=FALSE)
